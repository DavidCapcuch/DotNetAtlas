        ...
        where TCommand : ICommand<TResponse>
    {
        ...
        public async Task<Result<TResponse>> HandleAsync(TCommand command, CancellationToken ct)
        {
            ...
            try
            {
                ...
                if (result.IsSuccess)
                {
                    CqrsInstrumentation.RecordCommandSuccess(commandName, elapsedMs);
                }
                else
                {
                    CqrsInstrumentation.RecordCommandFailure(commandName, elapsedMs, ExtractDomainErrors(result));
                }
            ...
            {
                var elapsedMs = Stopwatch.GetElapsedTime(startTimeStamp).TotalMilliseconds;
                CqrsInstrumentation.RecordCommandException(commandName, elapsedMs, ex);
                throw;
            }
        }
    }
        ...
        where TQuery : IQuery<TResponse>
    {
        ...
        public async Task<Result<TResponse>> HandleAsync(TQuery query, CancellationToken ct)
        {
            ...
            try
            {
                ...
                if (result.IsSuccess)
                {
                    CqrsInstrumentation.RecordQuerySuccess(queryName, elapsedMs);
                }
                else
                {
                    CqrsInstrumentation.RecordQueryFailure(queryName, elapsedMs, ExtractDomainErrors(result));
                }
            ...
            {
                var elapsedMs = Stopwatch.GetElapsedTime(startTimeStamp).TotalMilliseconds;
                CqrsInstrumentation.RecordQueryException(queryName, elapsedMs, ex);
                throw;
            }
        }
    }
