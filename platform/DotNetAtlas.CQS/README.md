...
A lightweight Command Query Separation (CQS) library for .NET applications. Provides a clean architecture pattern for separating read and write operations with built-in support for cross-cutting concerns through decorator-based behaviors.
...
services.AddCqsHandlersFromAssembly(typeof(Program).Assembly);
...
```csharp
using DotNetAtlas.CQS.Observability;

services.AddOpenTelemetry()
    .WithMetrics(metrics => metrics
        .AddMeter(CqsInstrumentation.MeterName))
    .WithTracing(tracing => tracing
        .AddSource(CqsInstrumentation.ActivitySourceName));
```
...
|----------|--------------|
| `AddCqsValidationBehavior()` | Runs FluentValidation before handler, returns `Result.Fail` on validation errors |
| `AddCqsLoggingBehavior()` | Structured logging of processing, completion, errors, exceptions |
| `AddCqsMetricsBehavior()` | OpenTelemetry metrics (counters, histograms) - meter: `DotNetAtlas.CQS` |
| `AddCqsTracingBehavior()` | OpenTelemetry activity spans with trace context propagation |

### Metrics Behavior detail

- Use meter name `DotNetAtlas.CqsInstrumentation.MeterName`.
...
- Use activity source name `CqsInstrumentation.ActivitySourceName`.
...
- [DotNetAtlas.SharedKernel](../DotNetAtlas.SharedKernel) - Error types for Result pattern
