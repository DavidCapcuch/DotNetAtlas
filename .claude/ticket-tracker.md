...
Tickets and PRDs live as GitHub Issues in `DavidCapcuch/DotNetAtlas`. Use the `gh` CLI for all
...
- **Default page size is 30** — `gh issue list` / `gh search` cap there unless you pass `--limit <n>`. Raise it before concluding a label, ticket or PR is absent.
...
**Correcting a `ready-for-agent` ticket: edit the body, don't just comment.** A dispatched agent
...
**PRs are not a request surface** — triage covers tickets only. Nothing arrives here as an external
contribution, so a PR is always implementation of an already-filed ticket, never a request to be
judged. GitHub shares one number space across tickets and PRs, so a bare `#42` may be either —
resolve with `gh pr view 42`, falling back to `gh issue view 42`.

When a skill says "publish to the ticket tracker", create a GitHub issue. When it says "fetch the
