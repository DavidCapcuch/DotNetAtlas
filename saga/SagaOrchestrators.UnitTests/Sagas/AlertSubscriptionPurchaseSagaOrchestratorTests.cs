...
public class AlertSubscriptionPurchaseSagaOrchestratorTests : IAsyncLifetime
{
    ...
    public async Task WhenPaymentCompletedThenActivated_ShouldTransitionToActivationCompleted()
    {
        ...
        var completedMessages = _fakeOutboxWriter.GetMessages<AlertSubscriptionPurchaseCompletedEvent>().ToList();
        using (new AssertionScope())
        {
            _fakeOutboxWriter.HasMessage<AlertSubscriptionPurchaseCompletedEvent>().Should().BeTrue(
                "AlertSubscriptionPurchaseCompletedEvent should be added to the outbox when purchase completes");
    ...
    public async Task WhenActivationFailed_WithoutCompensation_ShouldTransitionToActivationFailed()
    {
        ...
        var failedMessages = _fakeOutboxWriter.GetMessages<AlertSubscriptionPurchaseFailedEvent>().ToList();
        using (new AssertionScope())
        {
            _fakeOutboxWriter.HasMessage<AlertSubscriptionPurchaseFailedEvent>().Should().BeTrue(
                "AlertSubscriptionPurchaseFailedEvent should be added to the outbox when activation fails without compensation");
    ...
    public async Task WhenCompensationCompleted_ShouldTransitionToCompensationCompleted()
    {
        ...
        {
            _fakeOutboxWriter.HasMessage<AlertSubscriptionPurchaseFailedEvent>().Should().BeTrue(
                "AlertSubscriptionPurchaseFailedEvent should be added to the outbox after compensation completed");
            failedMessages.Should().ContainSingle();
            failedMessages[0].IntegrationEvent.CorrelationId.Should().Be(correlationId);
    ...
    public async Task WhenCompensationTimeout_ShouldTransitionToCompensationFailed()
    {
        ...
        {
            _fakeOutboxWriter.HasMessage<AlertSubscriptionPurchaseFailedEvent>().Should().BeTrue(
                "AlertSubscriptionPurchaseFailedEvent should be added to the outbox after compensation timeout");
            failedMessages.Should().ContainSingle();
            failedMessages[0].IntegrationEvent.CorrelationId.Should().Be(correlationId);
            failedMessages[0].IntegrationEvent.CompensationTriggered.Should().BeTrue();
