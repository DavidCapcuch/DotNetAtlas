...
/// </summary>
/// <remarks>
/// The outbox binds this section onto a plain <see cref="AvroSerializerConfig"/>, which reads the
/// base string dictionary rather than the CLR properties — which is why every setting is bound by
/// its <see cref="AvroSerializerConfig"/> property name and none is redeclared here. Redeclaring one
...
/// the subject name strategy (<c>TopicName</c>, <c>RecordName</c>, <c>TopicRecord</c>) decides which
/// schema a consumer resolves, and auto-registration — local environment only, nowhere else —
/// decides whether an unknown schema is created or refused.
/// </summary>
