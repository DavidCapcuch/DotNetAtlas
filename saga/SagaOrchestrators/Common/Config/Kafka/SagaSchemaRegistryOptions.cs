...
/// <summary>
/// Startup validation for <see cref="SagaSchemaRegistryOptions"/>. <c>Url</c> (e.g.
/// <c>http://localhost:8081</c>) lives on the <see cref="SchemaRegistryConfig"/> base, where a data
/// annotation cannot reach it without the <c>new</c> redeclaration this type deliberately does not
/// carry.
/// </summary>
internal sealed class SagaSchemaRegistryOptionsValidator : IValidateOptions<SagaSchemaRegistryOptions>
{
    /// <summary>
    ...
    public ValidateOptionsResult Validate(string? name, SagaSchemaRegistryOptions options)
    {
    ...
    private static ValidateOptionsResult Fail(string problem) =>
        ValidateOptionsResult.Fail(
            $"Schema Registry configuration error in section '{SagaSchemaRegistryOptions.Section}': {problem}");
}
