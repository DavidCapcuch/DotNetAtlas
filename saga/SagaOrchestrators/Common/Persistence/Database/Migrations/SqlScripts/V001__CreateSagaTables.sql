...
DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM saga."__EFMigrationsHistory" WHERE "migration_id" = '20260604200529_CreateSagaTables') THEN
        IF NOT EXISTS(SELECT 1 FROM pg_namespace WHERE nspname = 'saga') THEN
            CREATE SCHEMA saga;
        END IF;
    END IF;
...
BEGIN
    ...
    COMMENT ON COLUMN saga.checkout_saga_state.correlation_id IS 'MassTransit saga instance id (ISaga.CorrelationId); equals the pre-assigned OrderId (ADR-0029).';
...
BEGIN
    ...
    COMMENT ON COLUMN saga.outbox_messages.type IS 'Avro type name of the serialized event (e.g., ''OrderConfirmedEvent'') for deserialization and observability';
...
BEGIN
    ...
    VALUES ('20260604200529_CreateSagaTables', '10.0.8');
    END IF;
END $EF$;
