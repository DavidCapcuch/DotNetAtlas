...
DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM saga."__EFMigrationsHistory" WHERE "migration_id" = '20260601002047_CreateSagaTables') THEN
        IF NOT EXISTS(SELECT 1 FROM pg_namespace WHERE nspname = 'saga') THEN
            CREATE SCHEMA saga;
        END IF;
    END IF;
...
BEGIN
    ...
    COMMENT ON COLUMN saga.payment_processing_saga_state.refund_timeout_token_id IS 'Token ID for refund timeout scheduler - set when schedule is active';
    COMMENT ON COLUMN saga.payment_processing_saga_state.success_finalization_timeout_token_id IS 'Token ID for success finalization timeout scheduler - set when schedule is active';
    END IF;
END $EF$;
...
BEGIN
    ...
    VALUES ('20260601002047_CreateSagaTables', '10.0.8');
    END IF;
END $EF$;
