...
/// </remarks>
public class SubscriptionPurchaseSagaTests : IAsyncLifetime
{
    private readonly FakeTimeProvider _fakeTimeProvider = new();
    private ServiceProvider _provider = null!;
    private ITestHarness _harness = null!;
    private ISagaStateMachineTestHarness<SubscriptionPurchaseSaga, SubscriptionPurchaseSagaState> _sagaHarness = null!;
    ...
    public async ValueTask InitializeAsync()
    {
        ...
        _provider = new ServiceCollection()
            .AddSingleton(Substitute.For<ILogger<SubscriptionPurchaseSaga>>())
        ...
        _sagaHarness = _harness.GetSagaStateMachineHarness<SubscriptionPurchaseSaga, SubscriptionPurchaseSagaState>();
    ...
    public async Task WhenSubscriptionPurchaseInitiated_ShouldTransitionToWaitingForPayment()
    {
        ...
        var initiatedEvent = new SubscriptionPurchaseInitiatedEvent
        {
        ...
        (await _sagaHarness.Consumed.Any<SubscriptionPurchaseInitiatedEvent>()).Should().BeTrue();

        // Wait for saga to exist
        var sagaExists = await _sagaHarness.Exists(correlationId, timeout: TimeSpan.FromSeconds(5));
        sagaExists.HasValue.Should().BeTrue("Saga should be created after publishing SubscriptionPurchaseInitiatedEvent");
    ...
    public async Task WhenPaymentCompletedThenActivated_ShouldTransitionToActivationCompleted()
    {
        ...
        (await _sagaHarness.Consumed.Any<SubscriptionActivatedEvent>()).Should().BeTrue();
        ...
        finalState.HasValue.Should().BeFalse("Saga should be finalized and removed from repository after activation completed");
    }
    ...
    public async Task WhenActivationFailed_WithCompensation_ShouldTransitionToCompensationInProgress()
    {
        ...
        var failedEvent = new SubscriptionActivationFailedEvent
        {
            CorrelationId = correlationId,
            UserId = userId,
            PaymentTransactionId = paymentTransactionId,
        ...
        // Assert
        (await _sagaHarness.Consumed.Any<SubscriptionActivationFailedEvent>()).Should().BeTrue();
    ...
    public async Task WhenActivationFailed_WithoutCompensation_ShouldTransitionToActivationFailed()
    {
        ...
        // Wait for saga to reach final state using proper MassTransit waiting
        var finalState = await _sagaHarness.NotExists(correlationId, timeout: TimeSpan.FromSeconds(5));
        finalState.HasValue.Should().BeFalse("Saga should be finalized and removed from repository after activation failed without compensation");
    }
    ...
    public async Task WhenCompensationCompleted_ShouldTransitionToCompensationCompleted()
    {
        ...
        // Wait for saga to reach final state using proper MassTransit waiting
        var finalState = await _sagaHarness.NotExists(correlationId, timeout: TimeSpan.FromSeconds(5));
        finalState.HasValue.Should().BeFalse("Saga should be finalized and removed from repository after compensation completed");
    }
    ...
    public async Task WhenPurchaseInitiated_ShouldInitializeAllStateProperties()
    {
        ...
        // Act
        await _harness.Bus.Publish(initiatedEvent);
        await _sagaHarness.Consumed.Any<SubscriptionPurchaseInitiatedEvent>();
        await _sagaHarness.Exists(correlationId, timeout: TimeSpan.FromSeconds(5));
