...
/// <summary>
/// Kafka consumer configuration for the inbound <c>StockLevelChanged</c> event from
/// Inventory. Bound from <c>KafkaStockLevelChangedConsumer</c>. Inherits from
/// <see cref="ConsumerConfig"/> so broker-level knobs (auto-offset-reset, session-timeout)
/// are bindable directly.
/// </summary>
public sealed class StockLevelChangedConsumerOptions : ConsumerConfig
{
    public const string Section = "KafkaStockLevelChangedConsumer";
    ...
    /// </summary>
    [Required(
        ErrorMessage = $"{nameof(GroupId)} for {nameof(StockLevelChangedConsumerOptions)} is missing",
