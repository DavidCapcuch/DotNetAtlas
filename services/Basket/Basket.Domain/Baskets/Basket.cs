...
public sealed class Basket : AggregateRoot<Guid>
{
    ...
    public BasketTotal? Total
    {
        get
        {
            ...
            // (enforced by ProductSnapshot.Create — Basket-local invariant, since Money does not enforce positivity).
    ...
    /// id (ADR-0029). Must not be <see cref="Guid.Empty"/>.
    /// </param>
    ...
    public Result Checkout(
        ...
        DateTimeOffset utcNow)
    {
        // Last-resort programmer-bug guards: the checkout command handler allocates the
