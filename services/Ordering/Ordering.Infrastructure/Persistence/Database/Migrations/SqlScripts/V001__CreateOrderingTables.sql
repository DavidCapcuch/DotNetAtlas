...
DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM ordering."__EFMigrationsHistory" WHERE "migration_id" = '20260531235507_CreateOrderingTables') THEN
        IF NOT EXISTS(SELECT 1 FROM pg_namespace WHERE nspname = 'ordering') THEN
            CREATE SCHEMA ordering;
        END IF;
    END IF;
...
BEGIN
    ...
    COMMENT ON TABLE ordering.inbox_messages IS 'Inbox pattern table for idempotent message processing. Tracks processed messages to prevent duplicate processing.';
    COMMENT ON COLUMN ordering.inbox_messages.message_id IS 'Unique message identifier (Primary Key).';
    COMMENT ON COLUMN ordering.inbox_messages.processed_at_utc IS 'UTC timestamp when the message was processed.';
    END IF;
END $EF$;
...
BEGIN
    ...
    COMMENT ON TABLE ordering.outbox_messages IS 'Outbox pattern table for storing domain events as Avro-serialized messages for reliable event publishing.';
    COMMENT ON COLUMN ordering.outbox_messages.id IS 'PK, Identity';
    COMMENT ON COLUMN ordering.outbox_messages.topic_name IS 'The Kafka topic where this message will be published. Set by the message producer.';
    COMMENT ON COLUMN ordering.outbox_messages.kafka_key IS 'Kafka Key - typically the Aggregate ID for proper event ordering and partitioning';
    COMMENT ON COLUMN ordering.outbox_messages.avro_payload IS 'Avro-serialized domain event payload';
    COMMENT ON COLUMN ordering.outbox_messages.type IS 'Avro type name of the serialized event (e.g., ''FeedbackChangedEvent'') for deserialization and observability';
    COMMENT ON COLUMN ordering.outbox_messages.headers IS 'JSON dictionary of OpenTelemetry-standard headers for distributed tracing and metadata. Headers are automatically generated by OpenTelemetry propagators for end-to-end trace context propagation.';
    COMMENT ON COLUMN ordering.outbox_messages.created_utc IS 'Creation timestamp (UTC).';
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM ordering."__EFMigrationsHistory" WHERE "migration_id" = '20260531235507_CreateOrderingTables') THEN
    CREATE INDEX ix_inbox_messages_processed_at_utc ON ordering.inbox_messages (processed_at_utc);
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM ordering."__EFMigrationsHistory" WHERE "migration_id" = '20260531235507_CreateOrderingTables') THEN
    CREATE INDEX ix_orders_buyer_id ON ordering.orders (buyer_id);
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM ordering."__EFMigrationsHistory" WHERE "migration_id" = '20260531235507_CreateOrderingTables') THEN
    CREATE INDEX ix_orders_buyer_id_created_at_utc ON ordering.orders (buyer_id, created_at_utc);
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM ordering."__EFMigrationsHistory" WHERE "migration_id" = '20260531235507_CreateOrderingTables') THEN
    CREATE UNIQUE INDEX ux_orders_correlation_id ON ordering.orders (correlation_id);
    END IF;
END $EF$;
...
BEGIN
    ...
    VALUES ('20260531235507_CreateOrderingTables', '10.0.8');
    END IF;
END $EF$;
