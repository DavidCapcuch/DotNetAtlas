...
/// arrive, at which point M7's <c>IssueInvoiceCommandHandler</c> reads the row
...
/// half writes <see cref="CompletedAtUtc"/>; M7 will additionally raise an
...
public sealed class PendingInvoice
{
    ...
    /// <summary>Buyer extracted from <c>OrderConfirmedEvent.BuyerId</c>; null until the order half arrives. M7's allocator partitions by buyer and the issued event keys on this.</summary>
    public Guid? BuyerId { get; set; }

    /// <summary>Full <c>OrderConfirmedEvent</c> serialised to JSON; null until the order half arrives.</summary>
    /// <remarks>PII per ADR-0011 — do not log. M7 hydrates the aggregate from this column; logging the raw envelope would leak the buyer name + (future) billing address.</remarks>
    ...
    /// <summary>Set by M7's <c>IssueInvoiceCommandHandler</c> after a successful issuance, atomically with the <c>Invoice</c> aggregate insert. Null in M6 — the M6 consumer never writes here.</summary>
