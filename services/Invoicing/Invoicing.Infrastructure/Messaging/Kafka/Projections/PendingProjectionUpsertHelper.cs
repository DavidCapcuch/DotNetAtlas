...
/// find-by-order-id-or-insert primitive so each handler stays focused on
...
internal static class PendingProjectionUpsertHelper
{
    ...
    public static async Task<(TRow Row, bool IsNew)> GetOrAddAsync<TRow>(
        ...
        where TRow : class
    {
        var existing = await set.FindAsync([orderId], ct);
