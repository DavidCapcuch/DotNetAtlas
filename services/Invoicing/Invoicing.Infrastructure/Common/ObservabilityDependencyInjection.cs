using Invoicing.Infrastructure.Common.Observability;
...
public static class ObservabilityDependencyInjection
{
    ...
    /// Sets up instrumentation for ASP.NET Core, HTTP clients, and EF Core. Redis /
    /// SignalR / FusionCache / Hangfire instrumentation is intentionally omitted —
    /// Invoicing does not consume those subsystems from its Infrastructure layer
    /// (Redis is wired in the Presentation layer for idempotency-key output cache only).
    /// </summary>
