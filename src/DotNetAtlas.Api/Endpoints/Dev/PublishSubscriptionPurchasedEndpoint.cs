...
using OrderSubscriptionTier = Order.AlertSubscriptions.SubscriptionTier;
...
internal class PublishSubscriptionPurchasedEndpoint : Endpoint<PublishSubscriptionPurchasedCommand>
{
    ...
    public override void Configure()
    {
        ...
        Summary(s =>
        {
            ...
            s.ExampleRequest = new PublishSubscriptionPurchasedCommand
            {
                ...
                SubscriptionTier = OrderSubscriptionTier.Pro,
                DurationDays = 30
            };
        });
    }
    ...
    {
        var avroCommand = new ActivateSubscriptionCommand
        {
            CorrelationId = req.CorrelationId,
            UserId = req.UserId,
            PaymentTransactionId = req.PaymentTransactionId,
            Tier = MapToWeatherTier(req.SubscriptionTier),
        ...
        await _devEventsProducer.PublishActivateSubscriptionCommandAsync(avroCommand);

        _logger.LogInformation(
            "Published ActivateSubscriptionCommand - " +
            "MessageId: {MessageId}, CorrelationId: {CorrelationId}, UserId: {UserId}, PaymentTransactionId: {PaymentTransactionId}," +
            " Tier: {Tier}, DurationDays: {DurationDays}",
            messageId, req.CorrelationId, req.UserId, req.PaymentTransactionId, req.SubscriptionTier, req.DurationDays);
        ...
        await Send.OkAsync(new
        {
            ...
            CorrelationId = req.CorrelationId,
            PaymentTransactionId = req.PaymentTransactionId,
            Topic = _topicsOptions.WeatherAlertsCommands,
            Message = "Command published successfully"
        }, ct);
    }

    private static WeatherSubscriptionTier MapToWeatherTier(OrderSubscriptionTier tier)
    {
        return tier switch
        {
            OrderSubscriptionTier.Pro => WeatherSubscriptionTier.Pro,
            OrderSubscriptionTier.Ultra => WeatherSubscriptionTier.Ultra,
