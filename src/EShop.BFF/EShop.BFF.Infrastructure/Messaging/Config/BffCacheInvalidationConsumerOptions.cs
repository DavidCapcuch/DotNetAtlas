...
/// <para>
/// The consumer group id — per the one-group-per-service rule (events-catalog.md § 3.1),
/// <c>bff-group</c>, the sole BFF consumer group across every topic it subscribes to — carries no
/// annotation and no validator: <c>AddKafka</c> builds the cluster during DI registration, and
/// <c>ConsumerConfiguration</c>'s constructor rejects a null or empty <c>GroupId</c> there —
/// strictly before options validation, which only runs once the host starts.
/// </para>
/// <para>
