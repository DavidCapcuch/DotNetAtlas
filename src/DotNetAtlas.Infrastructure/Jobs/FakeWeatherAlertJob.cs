...
internal sealed class FakeWeatherAlertJob : IJob
{
    ...
    public static string JobId(string id) => $"{nameof(FakeWeatherAlertJob)}-{id}";

    private readonly ICommandHandler<SendWeatherAlertCommand> _sendWeatherAlertHandler;
    private readonly ILogger<FakeWeatherAlertJob> _logger;

    public FakeWeatherAlertJob(
        ILogger<FakeWeatherAlertJob> logger,
