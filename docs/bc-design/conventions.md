...
- **Externally-standardized media types are shared** — RFC 9457 `ProblemDetails`, emitted centrally by `Platform.Api`'s response sender. The qualifying property is that an **external standard froze the shape**, so no endpoint *can* have a reason to change it — *not* that the type looks generic.
- **A paging envelope is endpoint-owned**, not a shared wrapper — as is any other container this repo authors, such as a batch envelope. This repo writes its own page shape, so it is a wire contract like any other: a search page has reasons to change (facet counts) that a category listing does not. Each paginated endpoint declares its own copy of the same four-member shape; only the item type varies. The shape, the rationale, and the revisit trigger: [ADR-0037 § Why a paging envelope is not a media type](../adr/0037-endpoint-owned-response-contracts.md).
