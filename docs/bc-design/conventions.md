...
- **Generic envelopes are shared too** — RFC 9457 `ProblemDetails`, `PagedResult<T>`-style wrappers. A structural container carries no contract of its own; the `T` it wraps does, and that stays endpoint-owned.
