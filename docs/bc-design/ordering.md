...
| **Primary pattern showcase** | Rich **SmartEnum-guarded status FSM** with multi-event aggregate transitions, factory from `BasketSnapshot`, Ardalis.Specification reads. |
...
| Query | Spec | Returns |
|-------|------|---------|
| `GetOrderByIdQuery(OrderId, BuyerId)` | `OrderByIdSpec` — `o.Id == id && o.BuyerId == buyerId` | Full order projection (items, addresses, status, timestamps). BuyerId is enforced for row-level authorization (a buyer may only read their own orders; admin variant bypasses). |
| `GetOrdersByBuyerQuery(BuyerId, OrderStatus?, Skip, Take)` | `OrdersByBuyerSpec` — `o.BuyerId == buyerId && (status == null || o.Status == status)`, `.OrderByDescending(o => o.CreatedAtUtc)`, `.Skip(skip).Take(take)` | Paged summary projection. |

Both specs inherit `Ardalis.Specification.Specification<Order>` and tag the query with `.TagWith(nameof(SpecType))` for distributed tracing.
