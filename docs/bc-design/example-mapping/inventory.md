...
### 📐 Rules
- **R1** — The default TTL is **15 minutes** from `ReservedAtUtc`; `ExpiresAtUtc` is written into `StockReservedDomainEvent` at reservation time and is an immutable fact on the stream.
...
- **R3** — Expiry MUST produce a real `ReservationReleasedDomainEvent` (with `ReleaseReason = Expiry`) on the stream and on `inventory.reservations` — there are NO silent state changes.
...
- **Verify** R1
- **Then** `ReservationConfirmedDomainEvent` is appended, `Status` becomes `Confirmed` in `reservation_audit`, and when the `ReservationExpiryWorker` ticks at `T0 + 15m` it does NOT select `R1` (status is no longer `Active`).
...
- **Verify** R2, R3
- **Then** the worker issues `ReleaseReservationCommand(R2, reason=Expiry)`, `ReservationReleasedDomainEvent(ReleaseReason=Expiry)` is appended at the next version, projections update `R2.Status = 'Released'`, and external `ReservationReleasedEvent(ReleaseReason=Expiry)` is published on `inventory.reservations`.
...
- **Verify** R4
- **Then** the aggregate sees `Reservations[R3].Status == Released`, and the command returns `Result.Fail(InventoryErrors.ReservationNotActive)` — no `ReservationConfirmedDomainEvent` is appended, the saga treats the response as a failure and fails the order.
...
- **R4** — Stock can never go negative by construction — `StockAdjustedDomainEvent` requires `(OnHand + delta) - Reserved >= 0` (§ 5.6); any attempt to drive stock below reservations throws `DataIntegrityException`.
...
- **Verify** R1, R2
- **Then** `StockReservedDomainEvent(P1, R1, qty=7, O1, ExpiresAtUtc=now+15m)` is appended at `Version=4`; projections show `{OnHand=10, Reserved=10, Available=0}`; external `StockReservedEvent` is published.
...
- **Verify** R6
- **Then** the reserve handler rehydrates the stream (sees `StockReceivedDomainEvent` at `Version=2`, `OnHand=10`), evaluates `Available=10 >= 5`, and appends `StockReservedDomainEvent` at `Version=3` — no stale projection reads gate the decision.
...
### 📐 Rules
- **R1** — `ConfirmReservationCommand` transitions a reservation from `Active` to `Confirmed` and appends `ReservationConfirmedDomainEvent` to the stream.
...
- **Verify** R1, R2
- **Then** `ReservationConfirmedDomainEvent(P1, R1)` is appended; projections show `{OnHand=7, Reserved=0, Available=7}`; `R1.Status=Confirmed` in `reservation_audit`; external `ReservationConfirmedEvent` is published on `inventory.reservations`.
