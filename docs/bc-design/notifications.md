...
> **Status:** v2 target design — agreed 2026-06-03. **Supersedes** the v1 command-driven single-channel implementation (one `SendEmailNotificationCommand` → render → email → `EmailNotificationSentEvent`, no domain model, no HTTP). The v1 contracts + `notifications.email-*` topics stayed physically present but **orphaned** during the v2 build (at the time still referenced only by `src/Weather`); they were removed together with the Weather reference service in the final cleanup ticket (#318). Authoritative decisions: [ADR-0031](../adr/0031-notify-user-command-and-notification-id.md) (contract + `NotificationId`) and [ADR-0032](../adr/0032-notifications-dispatch-and-channels.md) (dispatch + channels). Preserves the command-driven direction of [events-catalog.md § 1.4 D-5](events-catalog.md). Built as a phased set of `ready-for-agent` tickets; until each phase lands, this doc describes the target, not the deployed state.
