...
The `bff` service client is already provisioned with all six scopes ([`service-scope-matrix.md`](../../src/keycloak/service-scope-matrix.md)); still outstanding are the BFF's exchange handler **and** the Keycloak-side enablement (turn on the `token-exchange` feature + grant the `bff` client permission to exchange for each callee) — both land when the BFF is built (the one unbuilt unit), see [roadmap.md § 2.3 BFF](../roadmap.md). **Load-bearing caveat:** Basket's FunctionalTests mint a synthetic token that fuses buyer `sub` + `basket-service` `aud` (`FakeTokenCreator` + the production-audience `FakeTokenSigner`) — a combination no real Keycloak flow currently produces — so their green status is **not** evidence the real exchange path works end-to-end. That proof lands with the BFF.
