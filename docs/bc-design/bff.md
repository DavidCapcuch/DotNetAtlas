...
- **Metrics** (OpenTelemetry meter `EShop.BFF`):
  ...
  - `bff.upstream.unbindable_payload` — counter, tagged `{ upstream }` — an upstream answered 2xx with a body the BFF could not bind to its anti-corruption record (§ 4). Strict binding routes that into the same degradation an outage produces, so without this counter `bff.partial_response` cannot distinguish *"the upstream is down"* from *"the upstream changed its contract"* — the two need opposite responses, and only the exception type in the log tells them apart. It is the standing alert candidate for the § 4 accepted gap: a non-zero rate means a contract drifted in production.
- **Upstream-call outcomes reuse standard instrumentation — no custom counter**, except the binding failure above, which none of it can see: the HTTP layer recorded a success and the resilience pipeline saw no timeout or circuit event. An earlier draft specified a `bff.upstream.calls{ client, outcome = success|timeout|5xx|circuit-open }` counter; those outcomes were dropped as redundant, since each is already emitted by meters the `AddMeter("*")` wildcard collects:
