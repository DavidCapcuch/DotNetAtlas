...
Result Checkout(Guid orderId, Address shipping, Address billing, Guid paymentMethodId, DateTimeOffset utcNow)
    → orderId is pre-assigned by the command handler (ADR-0029)
...
**The ceilings are the downstream consumers', not Catalog's.** Ordering rebuilds the snapshot from the checkout event, and Invoicing rebuilds it again from `OrderConfirmedEvent`; each rejects anything over its own limit, so those are what Basket must reject. Sizing to Catalog instead would fail add-item for a widened Catalog value the consumers would have accepted. The chain spans **four** contexts — `Catalog.Domain` (`Sku` / `ProductName`) → Basket → `Ordering.Domain`'s `ProductSnapshot` → `Invoicing.Domain.Common.ValueObjects.Sku` — and **two hops have zero margin**: `Name` between Catalog and Ordering, and `Sku` between Ordering and Invoicing. Either is breached by a single widening anywhere upstream of it.

The Invoicing hop is the dangerous one. `OrderConfirmedEvent` is what the checkout saga treats as the capture pivot, so an over-long `Sku` fails *after* payment is taken — worse than the pre-payment stranding this guard exists to prevent, and past the point where compensation is cheap.

Two further traps for anyone moving a limit. Ordering enforces its own twice — in `ProductSnapshot` and in `CreateOrderCommandValidator`, which holds separate literals and runs first — so raising one without the other moves nothing. And the chain breaks from either direction: a consumer *tightening* below Basket is the quieter failure, since Basket goes on accepting values that consumer has started to refuse while the presence of a ceiling implies the case is covered. Nothing enforces any of this automatically, because no bounded context can see all four ends.

That chain bounds the **ACL** only. Rehydration is not covered by it — a persisted over-long value would breach the ceiling with Catalog nowhere in the picture, and fails closed as `BasketErrors.Corruption` exactly as a blank does. Length is measured **untrimmed** so the two producers stay safe in the same direction: Ordering trims before measuring, so raw ≤ the ceiling implies trimmed ≤ it, and Basket can never pass a value Ordering would refuse.
...
- `CheckoutBasketCommand(UserId, ShippingAddress, BillingAddress, PaymentMethodId)` — the Order's `OrderId` is pre-assigned server-side ([ADR-0029](../adr/0029-order-keyed-saga-and-pre-assigned-orderid.md)), not supplied by the caller. Basket is a **pass-through courier** for addresses and payment method per [ADR-0005](../adr/0005-customer-data-in-ordering.md); it validates only basic shape (non-empty strings, ISO 3166-1 alpha-2 country code) and stamps the values straight into `BasketCheckoutInitiatedEvent`. Exact field spec in [events-catalog.md § 5.2.1](events-catalog.md) and [use-cases.md § 2.1.6](use-cases.md).
...
- **Kafka topic (via `kafka-init`):** `basket.sessions` — partitions / retention / class are canonical in [kafka-topology.md](../kafka-topology.md) (`saga-state` class, 30-day retention); producer / consumers / key in [events-catalog.md § 2](events-catalog.md).
