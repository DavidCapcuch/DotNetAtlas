...
1. `CheckoutBasketCommand { UserId, CorrelationId }` — `CorrelationId` is generated by the caller (Api layer or BFF) via `Guid.CreateVersion7()`. It becomes the Checkout Saga `CorrelationId`.
2. Handler loads the basket.
3. `basket.Checkout(correlationId)` is invoked:
   - Invariant: `Items.Count > 0`. Empty-basket checkout fails with `BasketErrors.EmptyBasket`.
   - Queues `BasketCheckedOutDomainEvent` with a full `BasketSnapshot` payload (all items, all snapshots, total).
...
| `BasketCheckedOutDomainEvent` | `Basket.Checkout` | `UserId`, `CorrelationId`, `Snapshot` (full `BasketSnapshot` VO: items, total), `OccurredOnUtc` |
