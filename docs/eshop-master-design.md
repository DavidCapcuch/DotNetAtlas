...
- [ ] **Identity fields** are present for saga correlation + audit: the primary aggregate id (and `OrderId` on order-scoped events — the saga business key per [ADR-0029](adr/0029-order-keyed-saga-and-pre-assigned-orderid.md)) plus an `*AtUtc` timestamp.
...
**Aggregate:** `Order` (BuyerId, Items, ShippingAddress, BillingAddress, Status, PaymentTransactionId, Total, timestamps). The Order's identity (`Id`) is the pre-assigned `OrderId` (UUID v7; [ADR-0029](adr/0029-order-keyed-saga-and-pre-assigned-orderid.md)), which is also the saga key.
...
- **Correlation key:** `OrderId` — the saga's MassTransit `CorrelationId` equals `BasketCheckoutInitiatedEvent.OrderId` (pre-assigned UUID v7; [ADR-0029](adr/0029-order-keyed-saga-and-pre-assigned-orderid.md)).
...
- **Health checks**: `AspNetCore.HealthChecks.Kafka`, `Microsoft.Extensions.Diagnostics.HealthChecks.EntityFrameworkCore`, `.Common` per service, plus `Platform.Kafka.TopicGuard`'s `Kafka topics` check — the one readiness check that probes no live dependency, reporting instead whether this instance ever verified that the topics it names exist. Standardized `/api/healthz` (liveness) and `/api/readiness` (readiness) — see [`Platform.ServiceDefaults.WebApplicationExtensions.MapPlatformHealthCheckEndpoints`](../platform/Platform.ServiceDefaults/WebApplicationExtensions.cs).

**Health probe contract.** Services *expose* `/api/healthz` (liveness) and `/api/readiness` (readiness) for an external prober to consume over HTTP — a pod-level `httpGet:`, an ALB target group, a monitoring system. No orchestrator manifests exist in this repo yet, so the only thing probing them today is the compose `HEALTHCHECK`, which uses `/api/readiness`. Readiness carries the dependency probes. **Liveness runs no checks on any host but the OutboxRelay**: it answers 200 whenever the process serves HTTP, so a green liveness response is not evidence that any dependency is healthy — see [`ServiceDefaultHealthCheckTags`](../platform/Platform.ServiceDefaults/Config/ServiceDefaultHealthCheckTags.cs).

**Base images.** Every service Dockerfile defaults `ARG BASE_IMAGE` to **`aspnet:10.0-noble-chiseled-extra`** (production: distroless, non-root, no shell or package manager — smallest attack surface), and [`docker-compose.yaml`](../docker-compose.yaml) overrides all services to **`aspnet:10.0-alpine3.22-extra`** through the single `x-bc-base-image` anchor (local-dev: busybox supplies the shell + `wget` the compose-level `HEALTHCHECK` against `/api/readiness` needs — which gives `docker compose ps` a `(healthy)` signal and enables `depends_on: condition: service_healthy` chains). Both are digest-pinned. The **`-extra`** feature is load-bearing on each: size-optimized .NET bases (plain alpine, chiseled, azure-linux-distroless) omit `icu`/`tzdata`, so `-extra` (which adds them) is required wherever the app uses globalization — e.g. the Notifications quiet-hours seed resolving `Europe/Prague` via `TimeZoneInfo` (ADR-0015).
...
### E.3 `CreateOrderCommand` partition key — `OrderId` (ADR-0029)

`OrderId` is **pre-assigned** at checkout initiation ([ADR-0029](adr/0029-order-keyed-saga-and-pre-assigned-orderid.md)), so it exists before `CreateOrderCommand` is published. All four Ordering commands are keyed by `OrderId`, and `CreateOrderCommand`'s idempotency check is an inline primary-key lookup on the pre-assigned `OrderId`. Every Ordering command co-partitions on `OrderId`.
...
**Note ([ADR-0029](adr/0029-order-keyed-saga-and-pre-assigned-orderid.md)):** `OrderId` **is** both the saga key and the lifecycle key, present on every Ordering external event including delivery. Correlate all lifecycle views by `OrderId`.
