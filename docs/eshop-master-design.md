...
- **Health checks**: `AspNetCore.HealthChecks.Kafka`, `Microsoft.Extensions.Diagnostics.HealthChecks.EntityFrameworkCore`, `.Common` per service. Standardized `/api/healthz` (liveness) and `/api/readiness` (readiness) — see [`Platform.ServiceDefaults.WebApplicationExtensions.MapPlatformHealthCheckEndpoints`](../platform/Platform.ServiceDefaults/WebApplicationExtensions.cs).
