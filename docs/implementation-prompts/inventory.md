...
- 6 ES events under `StockItems/Events/`: `StockItemInitializedDomainEvent`, `StockReceivedDomainEvent`, `StockReservedDomainEvent`, `ReservationConfirmedDomainEvent`, `ReservationReleasedDomainEvent`, `StockAdjustedDomainEvent` — all `sealed record : DomainEvent` with `required init` props; fields match `inventory.md § 5.1-5.6` verbatim
...
- ~~Service-auth on saga-command consumers~~ — **dropped per ADR-0010**: per-message `X-Service-Token` validation is the wrong layer. Kafka command topics carry no service-token auth; trust boundary = the deployment network (ADR-0009 reference profile).
...
| Kafka-side service-auth (X-Service-Token) | **Dropped entirely** — user removed `X-Service-Token` from Kafka headers as bad design. DoD line 314 wording reconciled to drop the validation. | Per ADR-0010: Kafka command topics carry no service-token auth; trust boundary = the deployment network (ADR-0009 reference profile), NOT per-message header validation. Ordering's saga-command handlers also don't validate any token. |
...
- **`test/Inventory.FunctionalTests/ApiEndpoints/StockItems/AdjustStockTests.cs`** — strengthened `WhenSameIdempotencyKeyReplayed_BothCallsReturn200` per the two M7 follow-ups (event-count strengthening + Redis-after-POST inspect):
    - Counts `stock_events` rows with `EventType=nameof(StockAdjustedDomainEvent)` before, after first POST, after second POST.
...
| Event-type assertions | `nameof(StockReservedDomainEvent)` etc. (post-review L3 fix) | The literal strings were correct but break silently on rename; `nameof` lets the compiler catch domain-event renames. Imports the `Inventory.Domain.StockItems.Events` namespace where missing. |
...
- **LOW-3 fixed**: replaced `"StockItemInitializedDomainEvent"` / `"StockReceivedDomainEvent"` / `"StockReservedDomainEvent"` / `"ReservationReleasedDomainEvent"` / `"ReservationConfirmedDomainEvent"` literal-string event-type comparisons with `nameof(...)` across all three new ExampleMapping test files. Compiler-enforced rename safety.
...
- [ADR-0010](../adr/0010-service-to-service-auth.md) — inbound JWT validation for admin endpoints (scope `inventory.commands.*`); saga-command Kafka consumers carry **no per-message `X-Service-Token` validation** — Kafka command topics carry no service-token auth; trust boundary = the deployment network (ADR-0009 reference profile). Catalog-event consumer likewise carries no message auth (event topic, not command).
