...
7. **H-2 / Money.Create symmetry** — In [`OrderItem.cs:60`](../../../services/Ordering/Ordering.Domain/Orders/ValueObjects/OrderItem.cs) and [`Order.cs:159`](../../../services/Ordering/Ordering.Domain/Orders/Order.cs), replace `new Money(...)` with `Money.Create(...)` + `Throw.If(result.IsFailed, ...)`.
