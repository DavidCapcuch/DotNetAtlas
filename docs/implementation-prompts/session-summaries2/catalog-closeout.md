...
| `Price.Amount > 0` | [`Product.cs` `Create` + `UpdatePrice`](services/Catalog/Catalog.Domain/Products/Product.cs) return `Result.Fail(ProductErrors.PriceMustBePositive())` on `<= 0`. Catalog-local invariant (post-School-B Money refactor): `Money` is a signed quantity, so positivity is enforced at the aggregate boundary rather than inherited. | `ProductTests` unit tests covering price-on-create and price-update paths. |
