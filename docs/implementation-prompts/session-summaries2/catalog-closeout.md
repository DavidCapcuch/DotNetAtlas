...
| MEDIUM | CAT-ARCH-C06 | [ApplicationDependencyInjection.cs:39](services/Catalog/Catalog.Application/Common/ApplicationDependencyInjection.cs) | `services.AddOptions<CatalogTopicsOptions>()` registers options but never binds them. Infrastructure binds at [MessagingDependencyInjection.cs:50](services/Catalog/Catalog.Infrastructure/Common/MessagingDependencyInjection.cs). If a future test boots Application-only, publishers will write `null` topic names until Kafka publish-time. | Drop the dangling `AddOptions` line OR move the `BindConfiguration` into Application DI (with a guard). |
