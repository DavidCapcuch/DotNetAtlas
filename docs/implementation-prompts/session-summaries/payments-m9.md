...
- **[ADR-0010](../../adr/0010-service-to-service-auth.md)** (service-to-service auth) — Admin HTTP endpoints under `/api/v1/payments/` are JWT-gated by `AuthPolicies.PaymentsAdmin` (scope `payments.read`). The `payments.commands` Kafka consumers run on PLAINTEXT broker per ADR-0010 lines 102-106 — **no per-message `X-Service-Token` validation in v1**; production hardening = SASL/OAUTHBEARER + per-service ACLs at the broker level. No outbound HTTP from Payments in v1 (gateway is stub, behind `IPaymentGateway` port — see § Gateway-error mapping).
...
- **Broker SASL/OAUTHBEARER + per-service Kafka topic ACLs** (per ADR-0010 lines 102-106 production-hardening note). Today `payments.commands` runs PLAINTEXT in v1 with no per-message service-token check — the production hardening path is documented but not implemented. Belongs to Wave 0 / Wave-N platform hardening.
