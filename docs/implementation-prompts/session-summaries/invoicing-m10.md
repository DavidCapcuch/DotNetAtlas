...
- **[ADR-0010](../../adr/0010-service-to-service-auth.md)** (service-to-service auth) — Admin HTTP endpoint under `/api/v1/invoicing/invoices/{id}/resend` is JWT-gated by `AuthPolicies.InvoicingAdmin` (scope `invoicing.admin.resend`). Buyer GET endpoints under `/api/v1/invoicing/invoices/...` + `/api/v1/invoicing/credit-notes/{id}` are gated by scope `invoicing.read`. The 4 Kafka projection consumers run on PLAINTEXT broker per ADR-0010 — no per-message `X-Service-Token` validation in v1; production hardening = SASL/OAUTHBEARER + per-service ACLs at the broker level. No outbound HTTP from Invoicing in v1 (only event consumption).
...
- **HTTP routes** under `/api/v1/invoicing/` per ADR-0012 — 5 endpoints (4 GET + 1 POST). Buyer GETs gated by `invoicing.read`; admin resend gated by `invoicing.admin.resend` + `.Idempotency()` per ADR-0013.
