...
| 3 saga consumers + Catalog `ProductCreatedEvent` + Ordering `OrderCancelledEvent`, ALL with inbox dedup; no message-level service-token auth per ADR-0010 | ✅ | [`MessagingDependencyInjection.cs:94-163`](../../../services/Inventory/Inventory.Infrastructure/Common/MessagingDependencyInjection.cs) — three `.AddConsumer(...)` blocks each wired `.AddInbox(typeof(...))`; no broker transport-auth config; matches ADR-0010 baseline. |
