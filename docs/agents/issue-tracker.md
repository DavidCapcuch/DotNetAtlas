...
**PRs as a request surface: no — triage covers issues only.** Nothing here arrives as an external contribution, so a PR is always implementation of an already-filed issue, never a request to be judged.
