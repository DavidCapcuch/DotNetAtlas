...
- **Share**: **standardized media types that no endpoint constructs** — RFC 9457 `ProblemDetails`, produced centrally by `Platform.Api`'s response sender. The qualifying property is *not* structural genericity, and *not* standardization on its own: RFC 9457 § 3.2 permits extension members, so the media type's shape is not frozen and this repo does populate it. What makes it shareable is that **no endpoint builds one** — a single central component does, so no endpoint has a surface on which to want it different. Duplicating it per endpoint would be absurd. A container an **endpoint constructs** does not qualify however generic it looks: a paging envelope and a batch envelope are endpoint-owned wire contracts (see *Why a paging envelope is not a media type* below).
...
- **The exemption presumes the designated namespace holds only value DTOs.** An *envelope* declared there is exempted by the anchor and escapes the rule entirely — so a BC's remediation must relocate every envelope out of it **before** the test can be trusted. Until that holds, a green run proves nothing about the types still sitting there.
- **The anchor is a namespace each BC designates, not the literal string `Common/Contracts`.** That path is Catalog's and Basket's; Inventory's shared types sit in `StockItems/Common`, the BFF's in `Api/Responses`, and Ordering / Invoicing / Payments declare theirs inline in a slice's response file. Each BC's arch test names its own designated namespace; a BC whose shared value DTOs are declared inside a slice must relocate them into one before the rule can be anchored at all.
- **Stop the reachability walk at the assembly boundary.** "Types reachable from the response" otherwise reaches `Guid`, `string`, `decimal`, `DateTimeOffset` and `IReadOnlyList<T>`, each referenced by many endpoints. The walk covers types declared in the BC's own assembly; BCL and framework types are outside the rule's subject.
