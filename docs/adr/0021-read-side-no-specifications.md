...
- **Drift between paged-list and single-detail projections of the same aggregate.** Where two handlers return the same response shape (Ordering's `GetOrdersByBuyer` + `GetOrderById`; Invoicing's three invoice queries), the projection is now duplicated inline (Ordering) or shared via an EF expression property (Invoicing's `InvoiceRow.Projection`). Future drift between the two Ordering projections is possible. Mitigation: a comment in each handler points at the sibling; functional tests pin the wire shape; a future Ordering handler that gains a third read path may want to follow the Invoicing pattern (shared `Expression<Func<...>>` projection).
