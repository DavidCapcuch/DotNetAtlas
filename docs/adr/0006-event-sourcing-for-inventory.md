...
- [kafka-topology.md](../kafka-topology.md) — `inventory.stock-events` and `inventory.reservations` are classified event-log (`retention.ms=-1`) so downstream BCs can replay-rebuild; per-topic table + rationale. Per-event producer/consumer in [events-catalog.md § 2](../bc-design/events-catalog.md).
