...
- [Kafka topology](../kafka-topology.md) — `inventory.stock-events` and `inventory.reservations` are classified as event-log (retention.ms=-1) so downstream BCs can replay-rebuild; rationale and per-topic table.
