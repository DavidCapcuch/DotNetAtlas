...
- The `PaymentId == CorrelationId` collapse (was out of scope of this ADR) — **resolved in cross-cutting wave1-followup #255**: the saga now mints a distinct UUID v7 `PaymentTransactionId` (carried on `AuthorizePaymentCommand`) and Payments uses it as the aggregate id, so `PaymentId` is independent of `CorrelationId`; one-payment-per-saga stays enforced by the unique index on `correlation_id` (see [payments.md § 2.2 I-7](../bc-design/payments.md)).
