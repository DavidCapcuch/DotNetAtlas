...
- **SignalR:** `INotificationBroadcaster` in Application, hub `/hubs/v1/notifications` in Api (versioned per the Weather `BasePaths` convention; Keycloak JWT; group = `RecipientUserId` via `Context.UserIdentifier`, joined in `OnConnectedAsync` / left in `OnDisconnectedAsync` — no client subscribe RPC, unlike Weather's per-location model). In-memory backplane only (omit Weather's mandatory `AddStackExchangeRedis`; Redis backplane is the multi-instance seam). A new `Notifications.FunctionalTests` project ports Weather's `SignalRClientFactory` + hub test client (the Weather one is bound to `WeatherAlertHub` — re-implement, not reference).
