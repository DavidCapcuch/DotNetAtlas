...
- **No doc-generator is added.** The interim Markdown is not the final state (see Future Direction); a bespoke generator would be throwaway.
- **Correctness of the physical facts is enforced at runtime, on three legs.** Compose creates the topics; `schema-registry-init` sets compat or fails loudly; and the broker runs with `auto.create.topics.enable=false` while every service verifies its own `GetAllTopics()` set against cluster metadata on its first readiness probe, reporting `Unhealthy` and naming the absent topics (`Platform.Kafka.TopicGuard`).
  - **The third leg catches one direction only.** `GetAllTopics()` is written against `kafka-create-topic`, so it detects a topic the compose block stopped provisioning. It cannot detect the reverse — a topic the *code* newly produces to or consumes from that was never added to `GetAllTopics()` — because nothing ties the list to the bus registration, which lives in a different project. That drift still surfaces the old way: `UNKNOWN_TOPIC_OR_PART` on a consumer, or a produce timeout. `test/Payments.ArchitectureTests/Infrastructure/TopicOptionsUsageTests.cs` is the in-repo precedent for the check that would close it.
