...
- [ADR-0030: Retire the Dedicated CorrelationId](0030-retire-dedicated-correlationid.md) — the dedicated `correlation.id` Kafka header sat **outside** the Avro schema, so the compatibility modes here never governed it (the basis for the old [ADR-0008](0008-correlation-id-propagation.md) note that "schema evolution does not affect propagation"). That header — and the dedicated id — is now retired, making the point moot for correlation. `OrderId` travels as an ordinary Avro payload field and **is** governed by the rules here.
