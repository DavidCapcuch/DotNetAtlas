...
- **Bootstrap**: After Schema Registry is up, a one-time admin task sets compatibility mode per subject. Sketch (implementation wave applies the real script):

  ```bash
  # For every event-log subject
  curl -X PUT http://schema-registry:8081/config/{Subject} \
       -H "Content-Type: application/vnd.schemaregistry.v1+json" \
       -d '{"compatibility": "FORWARD_TRANSITIVE"}'

  # For every command subject
  curl -X PUT http://schema-registry:8081/config/{Subject} \
       -H "Content-Type: application/vnd.schemaregistry.v1+json" \
       -d '{"compatibility": "FULL_TRANSITIVE"}'
  ```

- **Global fallback**: set Registry global compatibility to `FORWARD_TRANSITIVE` as the conservative default for any future subject that lacks explicit config.
