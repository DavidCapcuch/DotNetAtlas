...
2. It allows a single record type to travel across multiple topics with one subject (rare but useful — e.g., `SendEmailNotificationCommand` (v1; renamed in ADR-0031) flows from multiple producers into `notifications.email-commands` (v1; renamed in ADR-0031)).
