...
10. **Auth (deployed backchannels — inbound JWT validation + outbound service tokens):** Every inbound-JWT edge — the seven BCs (Catalog, Basket, Ordering, Inventory, Payments, Invoicing, Notifications) and the BFF — **fails closed** in any deployed environment (the published image defaults to `Production`): the host **refuses to start** until *both* of these override the local-dev defaults in the base `appsettings.json`:
