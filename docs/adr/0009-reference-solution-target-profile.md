...
10. **Auth (deployed backchannels — inbound JWT validation + outbound service tokens):** Every inbound-JWT edge — the seven BCs (Catalog, Basket, Ordering, Inventory, Payments, Invoicing, Notifications) and the BFF — **fails closed** in any deployed environment (the published image defaults to `Production`): the host **refuses to start** unless the metadata backchannel is HTTPS end to end.
    - `Authentication__JwtBearer__RequireHttpsMetadata` — base `appsettings.json` ships `true`, so a deployed environment inherits the secure value and needs **no override**. `appsettings.Development.json` and `appsettings.Testing.json` relax it to `false` for the local http Keycloak. Note the precedence: every edge binds `Authentication:JwtBearer`, and that bind runs *after* the platform default (`IsDeployedEnvironment()`) — so the base key always wins, and the platform default applies only to a host that pins `RequireHttpsMetadata` nowhere. An edge shipping the base `true` **must** therefore carry both relaxing overlays *in the source tree*, or it fails to boot under Development and Testing. Note what reaches a deployed image: `appsettings.Development.json` ships (docker-compose runs every service as `Development`), but `appsettings.Testing.json` is deliberately excluded from publish (root `Directory.Build.targets`, `CopyToPublishDirectory=Never`) — it exists only for in-process test fixtures, and shipping it would let a container started with `ASPNETCORE_ENVIRONMENT=Testing` inherit the relaxation rather than fail closed.
    - `Authentication__JwtBearer__Authority` → a reachable `https://` OIDC endpoint whose TLS chain the service trusts. Base `appsettings.json` is deployment-shaped: it ships **no** `Authority` (the dev-only `http://localhost` value lives in `appsettings.Development.json` / `appsettings.Testing.json`), so a deployed host that omits the key fails closed at boot via the platform's `AssertDeployedAuthorityConfigured` guard — with neither `Authority` nor `MetadataAddress` the framework builds no configuration manager and, without this guard, would boot cleanly and defer failure to the first authenticated request. The framework's own metadata-address check is belt-and-suspenders for the narrower case of an explicit plaintext `Authority` under `RequireHttpsMetadata=true` (a deployed override pointing at `http://`), which it rejects at boot.

    Every one of these misconfigurations fails **at startup**, during the `ValidateOnStart` host materialization, so a host that starts has cleared them all: the platform guards trip on `RequireHttpsMetadata=false` (reachable only if a deployed environment explicitly re-disables it) and on a missing `Authority`; an `http://` authority under `RequireHttpsMetadata=true` is rejected by the framework's own metadata-address check; and the outbound `ServiceAuth` values fail their `[Required]` / https validation. Only an *unreachable or untrusted* `https` backchannel surfaces later, on the first authenticated request — so smoke-test a real authenticated request after deploy, not just a boot/health probe. `RequireHttpsMetadata` gates the service→Keycloak metadata/JWKS *backchannel* — not inbound request TLS, which you terminate at your ingress (the planned YARP edge is not yet built; see ADR-0035). The guard lives once in the platform `JwtBearerConfigurator` (`AddPlatformJwtBearer`, wired with `ValidateOnStart`) and applies uniformly to every edge — there is no per-BC guard. An internal-plaintext-metadata topology (Keycloak reachable only over cluster-internal `http://`) is **out of scope**: the guard admits no exception, so an adopter who needs it relaxes the platform guard themselves, accepting the plaintext metadata/JWKS MITM surface.

    **Outbound service tokens (deployed):** the same TLS requirement applies to the outbound backchannel on every *outbound-active* edge — the services that call `services.AddServiceAuth(...)` (Basket and the BFF today). `ServiceAuth__Authority` must be a reachable `https://` OIDC realm URL; base `appsettings.json` is deployment-shaped and ships **no** `ServiceAuth:Authority` (the dev-only `http://localhost` value lives in the Development / Testing overlays). The token endpoint derived from it receives the client-credentials `client_secret` and the RFC 8693 exchanged user tokens, so a plaintext Authority POSTs those secrets in the clear. `ServiceAuthOptions.Authority` is `[Required]` in *every* environment — so the Development / Testing overlays must supply the http value — while the additional **https** scheme check is deployed-only; a deployed host that sets no `ServiceAuth:Authority`, or an `http://` one, fails boot via the `ValidateOnStart` guard on `ServiceAuthOptions`: the outbound symmetry of the inbound guard above. The same internal-plaintext-metadata topology stays out of scope (relax the platform guard to accept it). See [ADR-0010](0010-service-to-service-auth.md).

11. **Dev surfaces (Swagger UI + developer exception page):** both gate on the environment *name*, never on a literal `Production` check — `IsDeployedEnvironment()` is true for every name other than `Development` / `Testing` — so a tier named `Staging` or `Dev` leaks neither the API surface, its schemas, nor a stack trace. Naming is the whole gate, not where the container runs: a deployed host started as `Testing` serves Swagger and un-redacted `Detail`, the same foot-gun that keeps `appsettings.Testing.json` out of the published image (item 10). The two surfaces reach that guarantee through **different** gates:
    - **Developer exception page** — `Development` only, via the platform `app.UsePlatformExceptionHandling()`. The narrower gate puts `Testing` on `PlatformExceptionHandler` too, so a test host exercises the RFC 9457 shape a deployed cluster serves — though not its body: `Detail` carries `Exception.Message` on any non-deployed tier and a redacted string on deployed ones. Deployed diagnostics go to structured logs → Seq / Jaeger, not an HTTP stack-trace page.
    - **Swagger UI** — each host serves it only when `!app.Environment.IsDeployedEnvironment()`, and that one call maps the OpenAPI document as well as the UI, so a deployed tier serves no document at all — the document is *registered* unconditionally in DI, but nothing routes it. Each host's `Pages/Index.cshtml` gates the landing-page link on the same predicate.
