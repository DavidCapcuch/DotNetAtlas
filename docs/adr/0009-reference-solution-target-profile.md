...
10. **Auth (deployed backchannels — inbound JWT validation + outbound service tokens):** Every inbound-JWT edge — the seven BCs (Catalog, Basket, Ordering, Inventory, Payments, Invoicing, Notifications) and the BFF — **fails closed** in any deployed environment (the published image defaults to `Production`): the host **refuses to start** unless the metadata backchannel is HTTPS end to end.
    ...
    - `Authentication__JwtBearer__Authority` → a reachable `https://` OIDC endpoint whose TLS chain the service trusts. Base ships the Development-tier `http://localhost` value, so a deployed host that forgets to override fails to boot on the framework's own metadata-address check (a plaintext address under `RequireHttpsMetadata=true`). The platform's `AssertDeployedAuthorityConfigured` guard covers the one residual case that check cannot see — an `Authority` explicitly blanked by an empty override, or an edge whose appsettings omits the key entirely — where the framework builds no configuration manager, the host boots cleanly, and failure is deferred to the first authenticated request.
