...
| DLT (Dead Letter Topic) | `{topic}.DLT` — destination for messages that failed all retries. 14-day retention; alert on cumulative count per topic. |
| Schema compatibility | **FORWARD_TRANSITIVE** for event-log topics (long-lived), **FULL_TRANSITIVE** for command topics. See ADR-0007. |
