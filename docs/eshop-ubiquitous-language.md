...
| Correlation key | `OrderId` (UUID v7) — the durable business key shared across a checkout workflow (one checkout → one order → one payment → one invoice); pre-assigned at checkout and also the saga's MassTransit `CorrelationId` ([ADR-0029](adr/0029-order-keyed-saga-and-pre-assigned-orderid.md)); `traceId` (W3C / OpenTelemetry) covers cross-cutting telemetry. |
