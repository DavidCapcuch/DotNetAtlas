...
public class CleanArchitectureLayerTests : BaseTest
{
    ...
    public void Domain_ShouldNotHaveDependencyOnAny_InfrastructurePackages()
    {
        ...
        result.FailingTypes.Should().BeEmpty(
            "Catalog.Domain must not reference EF Core, KafkaFlow, FastEndpoints or Redis " +
    ...
    /// forbidden set: <c>ICatalogDbContext</c> inherits <c>IOutboxDbContext</c>, which exposes
    ...
    public void Application_ShouldNotHaveDependencyOnAny_InfrastructurePackages()
    {
        ...
        result.FailingTypes.Should().BeEmpty(
            "Catalog.Application must not reference KafkaFlow, FastEndpoints or Redis — those are " +
            "Infrastructure/Api concerns (architecture-tests.md § 1.1): {0}",
            string.Join(", ", result.FailingTypes?.Select(t => t.Name) ?? []));
    }
}
