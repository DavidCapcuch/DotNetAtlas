...
public class GetPaymentsByOrderTests : BaseApiTest
{
    ...
    {
        var orderId = Guid.CreateVersion7();
        var seeded = await PaymentSeed.InsertRequestedAsync(DbContext, App.FakeTime.GetUtcNow(), orderId: orderId);
    ...
    {
        var orderId = Guid.CreateVersion7();
        var seedA = await PaymentSeed.InsertRequestedAsync(DbContext, App.FakeTime.GetUtcNow(), orderId: orderId);
        var seedB = await PaymentSeed.InsertRequestedAsync(DbContext, App.FakeTime.GetUtcNow(), orderId: orderId);
