...
/// <c>Payments.Domain.{Aggregate}.Events</c>.
/// </summary>
public class DomainEventTests : BaseTest
{
    ...
    public void DomainEvents_Should_LiveInAggregateEventsNamespace()
    {
        var result = Types.InAssembly(DomainAssembly)
            ...
            .Inherit<DomainEvent>()
            .Should()
            .ResideInNamespaceMatching(@"^Payments\.Domain\.\w+\.Events$")
            .GetResult();

        result.FailingTypes.Should().BeEmpty(
            "Domain events should live under 'Payments.Domain.<Aggregate>.Events' for predictable discovery");
    }
}
