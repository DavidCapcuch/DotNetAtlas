...
namespace Payments.ArchitectureTests.CleanArchitecture;

/// <summary>
/// Per architecture-tests.md § 1.1, enforces the four-layer topology
/// (Payments.Domain ← Payments.Application ← Payments.Infrastructure ← Payments.Api) — no upward
/// or sideways leaks.
/// </summary>
public class CleanArchitectureLayerTests : BaseTest
{
    ...
    public void Domain_ShouldNotHaveDependencyOnAny_InfrastructurePackages()
    {
        ...
        result.FailingTypes.Should().BeEmpty(
            "Payments.Domain must not reference EF Core, KafkaFlow, FastEndpoints or Redis " +
    ...
    /// forbidden set: <c>IPaymentsDbContext</c> inherits <c>IOutboxDbContext</c>, which exposes
    ...
    public void Application_ShouldNotHaveDependencyOnAny_InfrastructurePackages()
    {
        ...
        result.FailingTypes.Should().BeEmpty(
            "Payments.Application must not reference KafkaFlow, FastEndpoints or Redis — those " +
