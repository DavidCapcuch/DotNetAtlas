...
/// <summary>
/// Shared base for Inventory architecture tests. Anchors the four layer assemblies via the
...
{
    protected static readonly System.Reflection.Assembly DomainAssembly = typeof(global::Inventory.Domain.IAssemblyMarker).Assembly;
    protected static readonly System.Reflection.Assembly ApplicationAssembly = typeof(global::Inventory.Application.IAssemblyMarker).Assembly;
    protected static readonly System.Reflection.Assembly InfrastructureAssembly = typeof(global::Inventory.Infrastructure.IAssemblyMarker).Assembly;
    protected static readonly System.Reflection.Assembly PresentationAssembly = typeof(global::Inventory.API.IAssemblyMarker).Assembly;
    ...
    /// future <c>UpdateAsync</c>, <c>DeleteAsync</c>, <c>RemoveAsync</c>, or <c>ReplaceAsync</c>
    /// added to the port or its implementation fails this rule and forces an explicit
    /// architectural conversation.
    /// </summary>
    protected sealed class PublicMethodsAreSubsetOfRule : ICustomRule
    {
        private readonly HashSet<string> _allowedMethodNames;

        public PublicMethodsAreSubsetOfRule(params string[] allowedMethodNames)
        {
            _allowedMethodNames = allowedMethodNames.ToHashSet(StringComparer.Ordinal);
        }
        ...
        public bool MeetsRule(TypeDefinition type)
        {
            foreach (var method in type.Methods)
            {
                if (method.IsConstructor || method.IsStatic || method.IsGetter || method.IsSetter)
                {
                    continue;
                }
