...
namespace Notifications.ArchitectureTests.CleanArchitecture;
...
public class CleanArchitectureLayerTests : BaseTest
{
    ...
    public void Domain_ShouldNotHaveDependencyOnAny_InfrastructurePackages()
    {
        ...
        result.FailingTypes.Should().BeEmpty(
            "Notifications.Domain must not reference EF Core, KafkaFlow, FastEndpoints or Redis " +
    ...
    /// forbidden set: <c>INotificationsDbContext</c> inherits <c>IOutboxDbContext</c>, which
    /// exposes <c>DbSet&lt;OutboxMessage&gt;</c> and <c>DatabaseFacade</c> — so Application
    /// references EF Core by design, and only the concrete DbContext belongs to Infrastructure.
    /// </summary>
    [Fact]
    public void Application_ShouldNotHaveDependencyOnAny_InfrastructurePackages()
    {
        ...
        result.FailingTypes.Should().BeEmpty(
            "Notifications.Application must not reference KafkaFlow, FastEndpoints or Redis — " +
            "those are Infrastructure/Api concerns (architecture-tests.md § 1.1): {0}",
            string.Join(", ", result.FailingTypes?.Select(t => t.Name) ?? []));
    }
}
