...
/// <summary>
/// Shared fixture exposing the four Invoicing layer assemblies for NetArchTest assertions, plus
...
/// <remarks>
/// <para>
/// This rewrite consolidates the previous per-file <c>Invoicing.ArchitectureTests.Rules</c>
...
public abstract class BaseTest
{
    ...
    /// kept BC-local because it encodes ADR-0011's PII allowlist — can reuse the walker.
    /// </remarks>
    ...
    /// state machines / lambdas. Also enforced on <c>Invoicing.Infrastructure.Pdf</c> +
    /// <c>Invoicing.Infrastructure.Blobs</c> so QuestPDF templates and SAS expiry signing stay
    /// deterministic per ADR-0019.
    /// </summary>
    ...
    /// </summary>
    /// <remarks>
    /// Exposed for cross-BC symmetry. Invoicing currently has a justified by-type coupling
    /// (<c>CreditNote.Create(Invoice originalInvoice, ...)</c>) — see the divergence note on
    /// <see cref="BaseTest"/> for the rationale. No Invoicing test consumes this rule today;
    /// the rule lands as a regression guard for any newly-introduced cross-aggregate type.
    /// </remarks>
