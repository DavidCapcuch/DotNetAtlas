...
public class CleanArchitectureLayerTests : BaseTest
{
    ...
    public void Domain_ShouldNotHaveDependencyOnAny_InfrastructurePackages()
    {
        ...
        result.FailingTypes.Should().BeEmpty(
            "Basket.Domain must not reference EF Core, KafkaFlow, FastEndpoints or Redis " +
    ...
    /// forbidden set: <c>IBasketDbContext</c> inherits <c>IOutboxDbContext</c>, which exposes
    ...
    public void Application_ShouldNotHaveDependencyOnAny_InfrastructurePackages()
    {
        ...
        result.FailingTypes.Should().BeEmpty(
            "Basket.Application must not reference KafkaFlow, FastEndpoints or Redis — those are " +
