...
/// <summary>
/// Shared base for Basket architecture tests. Anchors the four layer assemblies and exposes the
/// custom <see cref="ICustomRule"/> implementations the rule set leans on. Mirrors
/// <c>test/Catalog.ArchitectureTests/BaseTest.cs</c>; see #217 for the cross-BC rollout.
/// </summary>
/// <remarks>
/// Pre-existing rules in this BC (kept in place):
/// <list type="bullet">
///   <item><see cref="PrivateConstructorsRule"/> — original Basket-only helper.</item>
///   <item><c>Basket.ArchitectureTests.Domain.TimePolicyTests</c> ships a stricter superset of
///   <see cref="NoStaticUtcNowRule"/> (also bans the bare <c>System.DateTime</c> type). It is
///   intentionally left in place — the Catalog-style rule below is added alongside it for
///   cross-BC symmetry, not as a replacement.</item>
/// </list>
/// </remarks>
public abstract class BaseTest
{
    ...
    /// IL is invisible to the rule set — see <see cref="OnlyThrowsRule"/> /
    /// <see cref="DoesNotThrowRule"/> / <see cref="NoStaticUtcNowRule"/>.
    /// </summary>
    ...
    /// IL references; this one limits to public/private surface area on the type itself.
    /// </summary>
    /// <remarks>
    /// Basket currently has a single aggregate, so no in-BC test consumes this rule today. The
    /// rule lands in <see cref="BaseTest"/> for symmetry with Catalog / Payments and as a
    /// regression guard the moment a second Basket aggregate ships. Note:
    /// <c>AggregateRootTests.AggregateRoots_ShouldNot_ReferenceOtherAggregatesByType</c> already
    /// covers the same intent at a coarser granularity via NetArchTest's
    /// <c>HaveDependencyOnAny</c>.
    /// </remarks>
